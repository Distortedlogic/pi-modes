...
1. Use the requested scope. If the scope is not clear, ask which scope:
   - Global: the package-owned `AGENT_MODES.yml` in `pi-just-answer`, declared in its `package.json` under `pi.modes`
   - Project: `<cwd>/.pi/AGENT_MODES.yml`
   - Package: a package-owned YAML file declared in `package.json` under `pi.modes`
2. For a global mode, edit the declared `AGENT_MODES.yml` in `pi-just-answer`.
