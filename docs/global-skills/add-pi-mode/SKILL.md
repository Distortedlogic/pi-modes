...
2. For a package mode, use an existing declared mode file when possible. Each `pi.modes` entry must be an exact relative `.yml` or `.yaml` file path. Do not use a glob or directory. Otherwise, create `AGENT_MODES.yml` in the package root and add `"./AGENT_MODES.yml"` to `pi.modes`. Preserve all other manifest fields and entries.
